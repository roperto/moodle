 ...
 */
class moodle1_workshepform_accumulative_handler extends moodle1_workshepform_handler {

    /** @var used by {@link self::migrate_legacy_scales()} */
    private $newscaleids = array();

    /**
     * Converts <ELEMENT> into <workshepform_accumulative_dimension>
     */
    public function process_legacy_element(array $data, array $raw) {
        ...
        $converted = (array)workshepform_accumulative_upgrade_element($fakerecord, $newscaleid, 12345678);
        unset($converted['workshepid']);

        $converted['id'] = $data['id'];
        $this->write_xml('workshepform_accumulative_dimension', $converted, array('/workshepform_accumulative_dimension/id'));

        return $converted;
    }

    /**
     * If needed, creates new standard (global) scale to replace the legacy workshep one and returns the mapping
     *
     * If the given $oldscaleid represents a scale, returns array $oldscaleid => $newscaleid that
     * can be used as a parameter for {@link workshepform_accumulative_upgrade_element()}. Otherwise
     * this method returns empty array.
     *
     * In workshep 1.x, scale field in workshep_elements had the following meaning:
     ...
     *   9 | Score out of 100
     *
     * @see workshepform_accumulative_upgrade_scales()
    ...
    protected function get_new_scaleid($oldscaleid) {
        ...
        if ($oldscaleid >= 0 and $oldscaleid <= 6) {
            ...
            if (!isset($this->newscaleids[$oldscaleid])) {
                ...
                // inform the workshep instance that it should annotate the new scale
    ...
    /**
     * Returns a definition of a legacy workshep scale
     *
     * @see workshepform_accumulative_upgrade_scales
    ...
    private function get_new_scale_definition($oldscaleid) {
        ...
        switch ($oldscaleid) {
        case 0:
            $data['name']  = get_string('scalename0', 'workshepform_accumulative');
            $data['scale'] = implode(',', array(get_string('no'), get_string('yes')));
            break;
        case 1:
            $data['name']  = get_string('scalename1', 'workshepform_accumulative');
            $data['scale'] = implode(',', array(get_string('absent', 'workshepform_accumulative'),
                                                get_string('present', 'workshepform_accumulative')));
            break;
        case 2:
            $data['name']  = get_string('scalename2', 'workshepform_accumulative');
            $data['scale'] = implode(',', array(get_string('incorrect', 'workshepform_accumulative'),
                                                get_string('correct', 'workshepform_accumulative')));
            break;
        case 3:
            $data['name']  = get_string('scalename3', 'workshepform_accumulative');
            $data['scale'] = implode(',', array('* ' . get_string('poor', 'workshepform_accumulative'),
                                                '**',
                                                '*** ' . get_string('good', 'workshepform_accumulative')));
            break;
        case 4:
            $data['name']  = get_string('scalename4', 'workshepform_accumulative');
            $data['scale'] = implode(',', array('* ' . get_string('verypoor', 'workshepform_accumulative'),
                                                '**',
                                                '***',
                                                '**** ' . get_string('excellent', 'workshepform_accumulative')));
            break;
        case 5:
            $data['name']  = get_string('scalename5', 'workshepform_accumulative');
            $data['scale'] = implode(',', array('* ' . get_string('verypoor', 'workshepform_accumulative'),
                                                '**',
                                                '***',
                                                '****',
            ...
            break;
        case 6:
            $data['name']  = get_string('scalename6', 'workshepform_accumulative');
            $data['scale'] = implode(',', array('* ' . get_string('verypoor', 'workshepform_accumulative'),
                                                '**',
                                                '***',
                                                '****',
...
/**
 * Transforms a given record from workshep_elements_old into an object to be saved into workshepform_accumulative
 *
 ...
 * @return stdclass to be saved in workshepform_accumulative
 */
function workshepform_accumulative_upgrade_element(stdclass $old, array $newscaleids, $newworkshepid) {
    $new = new stdclass();
