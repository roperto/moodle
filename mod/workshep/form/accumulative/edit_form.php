 ...
 * @uses moodleform
 */
class workshep_edit_accumulative_strategy_form extends workshep_edit_strategy_form {
    ...
    protected function definition_inner(&$mform) {
        ...
        for ($i = 0; $i < $norepeats; $i++) {
            $mform->addElement('header', 'dimension'.$i, get_string('dimensionnumber', 'workshepform_accumulative', $i+1));
            ...
            $mform->addElement('editor', 'description__idx_'.$i.'_editor',
                    get_string('dimensiondescription', 'workshepform_accumulative'), '', $descriptionopts);
            ...
            $mform->addElement('modgrade', 'grade__idx_'.$i,
                    get_string('dimensionmaxgrade','workshepform_accumulative'), null, true);
            $mform->setDefault('grade__idx_'.$i, 10);
            $mform->addElement('select', 'weight__idx_'.$i,
                    get_string('dimensionweight', 'workshepform_accumulative'), workshep::available_dimension_weights_list());
            $mform->setDefault('weight__idx_'.$i, 1);
        }

        $mform->registerNoSubmitButton('noadddims');
        $mform->addElement('submit', 'noadddims', get_string('addmoredimensions', 'workshepform_accumulative',
                workshep_accumulative_strategy::ADDDIMS));
        $mform->closeHeaderBefore('noadddims');

        $mform->addElement('header', 'configheader', get_string('configuration', 'workshepform_accumulative'));
        $algos = array();
        $algos[] = $mform->createElement('radio', 'config_algorithm', '',
                get_string('algomean', 'workshepform_accumulative'), 0);
        $algos[] = $mform->createElement('radio', 'config_algorithm', '',
                get_string('algoadditive', 'workshepform_accumulative'), 1);
        $mform->addGroup($algos, 'algos', get_string('algorithm', 'workshepform_accumulative'), array('<br />'), false);
