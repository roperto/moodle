 ...
 */
class moodle1_workshepform_rubric_handler extends moodle1_workshepform_handler {
    ...
    /**
     * Prepare to gather legacy elements info for a new workshep instance
     */
    ...
    protected function convert_legacy_criterion_elements() {
        ...
        $this->write_xml('workshepform_rubric_config', array('layout' => 'list'));
        ...
        $this->xmlwriter->begin_tag('workshepform_rubric_dimension', array('id' => $firstelement['id']));
        $this->xmlwriter->full_tag('sort', 1);
        $this->xmlwriter->full_tag('description', trim(get_string('dimensionnumber', 'workshepform_rubric', '')));
        $this->xmlwriter->full_tag('descriptionformat', FORMAT_HTML);

        foreach ($this->elements as $element) {
            $this->write_xml('workshepform_rubric_level', array(
                'id'               => $element['id'],
                'grade'            => $element['maxscore'],
                'definition'       => $element['description'],
                'definitionformat' => FORMAT_HTML
            ), array('/workshepform_rubric_level/id'));
        }

        $this->xmlwriter->end_tag('workshepform_rubric_dimension');
    }
    ...
    protected function convert_legacy_rubric_elements() {
        $this->write_xml('workshepform_rubric_config', array('layout' => 'grid'));

        foreach ($this->elements as $element) {
            $this->xmlwriter->begin_tag('workshepform_rubric_dimension', array('id' => $element['id']));
            ...
            foreach ($this->rubrics[$element['id']] as $rubric) {
                ...
                $level = (array)workshepform_rubric_upgrade_rubric_level($fakerecord, $element['id']);
                unset($level['dimensionid']);
                $level['id'] = $this->converter->get_nextid();
                $this->write_xml('workshepform_rubric_level', $level, array('/workshepform_rubric_level/id'));
            }
...
/**
 * Transforms given record into an object to be saved into workshepform_rubric_levels
 *
 * This is used during Rubric 1.9 -> Rubric 2.0 conversion
 *
 * @param stdClass $old legacy record from joined workshep_elements_old + workshep_rubrics_old
 * @param int $newdimensionid id of the new workshepform_rubric dimension record to be linked to
 * @return stdclass to be saved in workshepform_rubric_levels
 */
function workshepform_rubric_upgrade_rubric_level(stdclass $old, $newdimensionid) {
    $new = new stdclass();
    $new->dimensionid = $newdimensionid;
    $new->grade = $old->rgrade * workshepform_rubric_upgrade_weight($old->eweight);
...
/**
 * Given old workshep element weight, returns the weight multiplier
 *
 * Negative weights are not supported any more and are replaced with weight = 0.
 * Legacy workshep did not store the raw weight but the index in the array
 * of weights (see $WORKSHOP_EWEIGHTS in workshep 1.x). workshep 2.0 uses
 ...
 */
function workshepform_rubric_upgrade_weight($oldweight) {
